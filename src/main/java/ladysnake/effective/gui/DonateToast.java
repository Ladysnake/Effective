package ladysnake.effective.gui;
...
public class DonateToast implements Toast {
	...
	public Visibility draw(MatrixStack matrices, ToastManager manager, long startTime) {
		...
		manager.getGame().textRenderer.draw(matrices, Text.literal("Wish to support Effective?"), 34, 7, -256);
		manager.getGame().textRenderer.draw(matrices, Text.literal("Get cool cosmetics for only 5€!"), 34, 18, -1);
		manager.getGame().textRenderer.draw(matrices, Text.literal("More info: illuminations.uuid.gg/donators").setStyle(Style.EMPTY.withColor(Formatting.GREEN)), 34, 29, -1);
